...
module.exports = {
    ...
    cachePlayerData: (playerName, data) => {
        ...
        client.setAsync(playerName, JSON.stringify(data), 'EX', 1800)
            // .then(res => console.log('set', res))
        client.getAsync(playerName)
            // .then( res => console.log('get after set', JSON.parse(res)))
        client.keysAsync('*')
            // .then( res => console.log('post set keys', res) )
    }
}
